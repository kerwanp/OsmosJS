...
description: Extend Osmos capabilities by creating custom renderers
---
...
A renderer is simply a function that receive a `OsmosNode` and returns a `boolean` telling if the renderer has handled the given node.
Custom renderers can be registered using the `OsmosRenderer.register` method.
