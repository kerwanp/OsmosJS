...
const initOptions = (options?: Options) => {
  ...
  return Object.fromEntries(
    Object.entries(defaultedOptions).map(([key, value]) => {
      if (
        ['root', 'title', 'titleInner', 'body', 'icon', 'foldIcon'].includes(key) &&
...
export const remarkCallout: Plugin<[Options?], mdast.Root> = (_options) => {
  ...
  return (tree, file) => {
    visit(tree, 'blockquote', (node) => {
      ...
      if (options.callouts && !options.callouts.includes(calloutData.type)) {
        const newCallout = options.onUnknownCallout(calloutData, file)
        if (!newCallout) return
      ...
      const titleNode: mdast.Blockquote | mdast.Paragraph = {
        type: !options.titleInner(calloutData, options) ? 'paragraph' : 'blockquote',
        data: {
      ...
      if (titleNode.type === 'paragraph') titleNode.children.push(...titleInnerNode.children)
